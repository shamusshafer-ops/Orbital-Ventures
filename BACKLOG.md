...
|39| Pad aborts that damage pad (repair time) | Risk depth | S | ★★ | 29 | M |Shipped 2026-07-24 — per-pad damage tracking, repair time scales with Pads level, only a catastrophic ascent-loss (no escape-tower save) triggers it|
...
|117| Solar System map improvement epic: A) truthful angle + Oort-excluded sizing, B) WASD/keyboard nav all 3 renderers, C) port live ship-tracking to Phaser+SVG | Map = dashboard + planner, "more connected" | M (per slice) | ★★★ | — | M |Slice A shipped 2026-07-24 (map2dAngle, Oort-exclusion, 980×620 canvas). Slice B shipped 2026-07-24 (WASD/arrows across 3D/Phaser/SVG + shared popout — see ROADMAP.md). Slice C scoped, not started|
