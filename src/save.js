...
const SAVE_VERSION=58; // v58: E4.4 persistent launch-vehicle hull registry (additive serial-numbered hulls
// + flight history; existing hangar entries are backfilled, never historical flights invented).
//
...
// comment by volume and the block impossible to diff. Moved verbatim on 2026-07-28; nothing was dropped.
//
// When you add persisted state: bump the constant above AND add an entry at the top of that doc. The
// convention throughout is lazy defaults — new fields are read through `||`/falsy guards so a pre-bump
// save loads unchanged and needs no migrate function. Write an explicit migrate*() only when a value must
// be TRANSFORMED rather than defaulted (see migrateWindowsToDays / migrateEraSeen / migrateEphemerisWindows).
