...
function render(){
  ...
  renderCCLeft(); // slice 4: the Basic-only focal guide lives in the left rail — rebuild on every scene
  renderRailPersistent(); // persistent right-rail accordion (Mission Control / Flight Plan / Objectives)
...
function renderCommandCenter(){
  ...
  renderExecOverview();
  renderCCProgress(); // user request: active research + build/launch progress, featured on their own instead of buried in a stat line
...
// Featured, always-visible progress card: active research (name + bar + time left) and any
// build/launch campaigns in progress (bar + ETA) or sitting finished in the hangar (Fly button).
// Pulled out of execOverview's cramped one-line "Active R&D: …" mention so both are easy to
// track without opening the R&D tab or the Infrastructure drill-down.
function renderCCProgress(){
  const el=$('ccProgress'); if(!el) return;
  const ar=state.activeResearch;
  const rNode=ar && RESEARCH.find(r=>r.id===ar.id);
  ...
  const nextLine=nextR?`<div class="dim" style="font-size:11px;margin-top:3px">📋 Next: ${nextR.name}</div>`:'';
  const researchHTML = (ar && rNode)
    ? `<div style="display:flex;justify-content:space-between;font-size:13px"><b>${rNode.name}</b><span class="dim">${fmtTimeLeft(ar.monthsLeft)} left</span></div>
       <div class="bar" style="margin:4px 0 0;height:6px"><div class="fill" style="width:${rPct}%;background:var(--dom-research)"></div></div>${nextLine}`
    : `<div class="dim" style="font-size:12px">No active project — <span style="text-decoration:underline;cursor:pointer" onclick="setTab('rnd')">start one on the R&amp;D tab</span>.</div>${nextLine}`;
  const orders=buildQueueList().filter(o=>o.started);
  const hangar=hangarList();
  const orderRows=orders.map(o=>{
    const pct=clampA(Math.round(100*(o.monthsTotal-o.monthsLeft)/Math.max(1,o.monthsTotal)),0,100);
    return `<div style="margin-top:8px"><div style="display:flex;justify-content:space-between;font-size:13px"><b>${o.name}</b><span class="dim">${fmtTimeLeft(o.monthsLeft)} left</span></div>
      <div class="bar" style="margin:4px 0 0;height:6px"><div class="fill" style="width:${pct}%;background:var(--dom-engineering)"></div></div></div>`;
  }).join('');
  const hangarRows=hangar.map(h=>`<div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;font-size:13px">
      <span>✓ <b>${h.name}</b> — ready to fly</span>
      <button class="btn ghost" style="padding:2px 8px;font-size:12px" onclick="launchFromHangar('${h.id}')">Fly ▸</button>
    </div>`).join('');
  const buildHTML = (orders.length||hangar.length) ? orderRows+hangarRows
    : `<div class="dim" style="font-size:12px">Nothing building — commit a launch from the Bench.</div>`;
  el.innerHTML = `<div style="display:flex;gap:24px;flex-wrap:wrap">
      <div style="flex:1;min-width:220px"><div class="cc-panel-h" style="margin:0 0 4px">⚗ Active Research</div>${researchHTML}</div>
      <div style="flex:1;min-width:220px"><div class="cc-panel-h" style="margin:0 0 4px">🏗 Build &amp; Launch</div>${buildHTML}</div>
    </div>`;
}
// top executive overview — "how is my company doing?"
function renderExecOverview(){
  ...
  const ar=s.research;
  const mh=state.metricHist||{}; // #28: dashboard trend buffers
  $('execOverview').innerHTML=`
    <div class="cc-hero">
      ...
      <div class="cc-net" style="text-align:right">
        <div class="dim" style="font-size:11px;text-transform:uppercase;letter-spacing:.12em">${domDot('economy')}Monthly net</div>
        <div style="color:${netCol};font-size:18px;font-weight:700">${s.net>=0?'+':''}${fM(s.net)}/mo</div>
        <div class="dim" style="font-size:11px">+${fM(s.income)} in · −${fM(s.overhead)} ovh${(s.empireOpex>0||s.loanInterest>0)?` <span style="color:var(--bad)">(incl.${s.empireOpex>0?` −${fM(s.empireOpex)} empire`:''}${s.loanInterest>0?` −${fM(s.loanInterest)} loan`:''})</span>`:''} · −${fM(s.payroll)} pay</div>
      </div>
    </div>
    <div class="exec-grid" style="margin-top:8px">
      <div class="exec-stat"><span class="k">${domDot('economy')}Capital</span><span class="v" style="color:${state.money<1.5?'var(--bad)':'var(--ink)'}">${fM(s.capital)}</span></div>
      <div class="exec-stat"><span class="k">${domDot('exploration')}Reputation</span><span class="v">${fI(s.rep)}</span></div>
      <div class="exec-stat"><span class="k">${domDot('research')}Science</span><span class="v" style="color:var(--dom-research)">${s.science}</span></div>
      <div class="exec-stat"><span class="k">${domDot('crew')}Staff</span><span class="v">${(state.staff||[]).length}</span></div>
      <div class="exec-stat"><span class="k">${domDot('engineering')}Flights</span><span class="v">${s.flights}${s.successRate!=null?` · ${s.successRate}%`:''}</span></div>
      <div class="exec-stat"><span class="k">${domDot('warn')}Public Support</span><span class="v" style="color:${s.moodColor}">${s.support}% <span style="font-size:12px;font-weight:400">${s.mood}</span></span></div>
    </div>
    <div class="exec-sparks adv-only">
      ${metricSpark('Capital', mh.money, {color:'var(--ignite)', label:'capital trend'})}
      ${metricSpark('Reputation', mh.rep, {color:'#6db3ff', label:'reputation trend'})}
      ${metricSpark('Public Support', mh.support, {min:0, max:100, color:'#caa3ff', label:'public support trend'})}
      ${metricSpark('Launch Success', mh.success, {min:0, max:100, color:'var(--ok)', label:'launch success-rate trend'})}
      ${metricSpark('Science', mh.science, {color:'var(--readout)', label:'science trend'})}
    </div>
