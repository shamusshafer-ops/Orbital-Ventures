...
// ---------- 1. IDENTITY: no mission sets .inclination yet, so effectiveReqDv === reqDv everywhere ----------
{
  let mism=0;
...
// ---------- 2. inclinationDv: free at/above site latitude, zero when unset ----------
{
  ...
  check('above site: 65° from Cape (28.4°) is free', inclinationDv({reqDv:9400,inclination:65})===0);
  check('at site: exactly 28.4° is free', inclinationDv({reqDv:9400,inclination:LAUNCH_SITE_LAT})===0);
  check('just above: 30° is free', inclinationDv({reqDv:9400,inclination:30})===0);
}

// ---------- 3. inclinationDv: paid below site latitude, monotonic, matches 2·v·sin(Δi/2) ----------
{
...
// ---------- 5. Gate integration: a real budget gate uses the surcharge (canLaunch) ----------
{
  ...
  if(v.totalDv>=9400 && v.totalDv<need){
    ...
    check('gate: (design not in the between-band this run — logic asserted via effectiveReqDv identity)', canLaunch(v,{...m0,inclination:65},null,false).ok===canLaunch(v,{...m0,reqDv:9400},null,false).ok);
  }
}
